...
def calc_single_object(index, current, fp_tp):
    global RESULTS_OBJECTS, TRUTH_ANNOTATIONS

    ovthresh = 0.5
    d, class_index = index[0], index[1]
    fp, tp = fp_tp[0], fp_tp[1]
    image = current['image_id']
    bbox = current['bbox']
    confidence = current['score']
    gt_bboxes = [e['bbox'] for e in TRUTH_ANNOTATIONS if e['category_id'] == class_index and e['image_id'] == image]
    gt = [e for e in TRUTH_ANNOTATIONS if e['category_id'] == class_index and e['image_id'] == image]
